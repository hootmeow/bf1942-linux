#!/usr/bin/env bash
# ---------------------------------------------------------------------------
#  Battlefield 1942 Linux Server - Arch Linux Setup Script
#
#  Features:
#    ✓ Arch Linux native ([multilib] pacman integration)
#    ✓ Zero system downgrades (isolated local library shims)
#    ✓ Smart IP detection (LAN, NAT, public scenarios)
#    ✓ Comprehensive input validation
#    ✓ CPU affinity & performance tuning
#    ✓ Multi-instance support & bf1942_manager.sh compatibility
#    ✓ Standalone or BFSMD modes
#
#  Usage:
#    Interactive: sudo ./arch_bfsmd_setup.sh [instance_name]
#    Unattended:  sudo ./arch_bfsmd_setup.sh <instance_name> --yes [options]
#    All options: sudo ./arch_bfsmd_setup.sh --help
#
#  Author: OWLCAT (adapted for Arch Linux)
# ---------------------------------------------------------------------------
...
detect_ip_addresses() {
    ...
    if command -v ip >/dev/null 2>&1; then
        local_ip=$(ip -4 addr show scope global 2>/dev/null | awk '$1=="inet"{split($2,a,"/"); print a[1]; exit}' || echo "")
    fi
    if [ -z "$local_ip" ]; then
...
check_resources() {
    ...
    local available_space_kb=$(df -P /home | tail -1 | awk '{print $4}')
...
if ! command -v pacman &> /dev/null; then
    log_error "This script requires pacman (Arch Linux)."
    exit 1
fi

if ! grep -q "^\s*\[multilib\]" /etc/pacman.conf; then
    log_error "[multilib] repository is not enabled in /etc/pacman.conf."
    echo "  Battlefield 1942 is a 32-bit application and requires [multilib]."
    echo "  Please enable it by uncommenting [multilib] and Include in /etc/pacman.conf:"
    echo ""
    echo "    [multilib]"
...
echo ""
echo -e "${C_ARMY}   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "   ${YELLOW}★${NC} ${BOLD}BATTLEFIELD 1942 · DEDICATED SERVER${NC} ${YELLOW}★${NC}  ${C_STEEL}[Arch Linux]${NC}"
echo -e "   ${DIM}enhanced multi-instance setup — safe · isolated · optimized${NC}"
echo -e "${C_ARMY}   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
if [ "$INSTALL_MODE" = "bfsmd" ]; then
    ...
    case "$OPT_IP" in
        ...
        local)
            if command -v ip >/dev/null 2>&1; then
                SERVER_IP=$(ip -4 addr show scope global 2>/dev/null | awk '$1=="inet"{split($2,a,"/"); print a[1]; exit}' || echo "")
            fi
            if [ -z "$SERVER_IP" ]; then
...
# Allow non-root users to inspect instance status and port info
chmod 755 "$BF_HOME"
mkdir -p "$BF_ROOT"

# ------------------------------------------------------------
# STEP 2: Dependencies (Arch Linux [multilib])
# ------------------------------------------------------------
log_step "2/8: Checking 32-bit system dependencies"

ARCH_PKGS=()
for pkg in lib32-glibc lib32-gcc-libs lib32-ncurses wget curl tar; do
...
if [ ${#ARCH_PKGS[@]} -gt 0 ]; then
    log_info "Installing missing dependencies: ${ARCH_PKGS[*]}..."
    pacman -S --needed --noconfirm "${ARCH_PKGS[@]}"
else
    log_success "All required multilib packages are already installed."
fi

touch /etc/bf1942_deps_installed 2>/dev/null || true
...
# ------------------------------------------------------------
# STEP 5: Post-Installation Configuration & Isolation
# ------------------------------------------------------------
log_step "5/8: Configuring server files and local libraries"

cd "$BF_ROOT"

# Set up isolated local library directory
mkdir -p "${BF_ROOT}/lib"
if [ -f "/usr/lib32/libncursesw.so.6" ]; then
    ln -sf /usr/lib32/libncursesw.so.6 "${BF_ROOT}/lib/libncurses.so.5"
    log_success "Created isolated ncurses compatibility link in ${BF_ROOT}/lib"
elif [ -f "/usr/lib32/libncurses.so.6" ]; then
    ln -sf /usr/lib32/libncurses.so.6 "${BF_ROOT}/lib/libncurses.so.5"
    log_success "Created isolated ncurses compatibility link in ${BF_ROOT}/lib"
fi

# Link the static BF1942 binary (eliminates legacy libstdc++5 dependency)
if [ -f "bf1942_lnxded.static" ]; then
    ln -sf bf1942_lnxded.static bf1942_lnxded
    log_success "Configured static binary (bf1942_lnxded -> bf1942_lnxded.static)"
elif [ -f "bf1942_lnxded.dynamic" ]; then
    ln -sf bf1942_lnxded.dynamic bf1942_lnxded
    log_warn "Static binary not found, using dynamic binary"
else
...
Environment=LD_LIBRARY_PATH=${BF_ROOT}/lib:/usr/lib32
...
if [ -z "$FIREWALL_MODE" ]; then
    if [ "$ASSUME_YES" -eq 1 ]; then
        ...
        read -r -p "Configure firewall rules? [y/N] " response
...
if [ "$FIREWALL_MODE" = "skip" ]; then
    ...
    log_info "No active UFW or firewalld detected; skipping firewall rules."
fi
...
echo "   ./bf1942_manager.sh status ${INSTANCE_NAME}"
