#!/usr/bin/env bash
# ---------------------------------------------------------------------------
#  Battlefield 1942 Linux Server - Unified Setup Script (CentOS)
#
#  Tested on:
#    - CentOS Stream 9
#
#  Usage:
#    Interactive: sudo ./centos_stream9_bfsmd_setup.sh [instance_name]
#    Unattended:  sudo ./centos_stream9_bfsmd_setup.sh <instance_name> --yes [options]
#    All options: sudo ./centos_stream9_bfsmd_setup.sh --help
#
...
if ! command -v dnf &> /dev/null; then
    log_error "This script requires dnf (CentOS Stream 9)."
    exit 1
fi
...
if [ -f /etc/os-release ]; then
    . /etc/os-release
    if [ "$ID" != "centos" ]; then
        log_warn "This script is intended for CentOS. Detected: ${PRETTY_NAME:-unknown}"
...
echo ""
echo -e "${C_ARMY}   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "   ${YELLOW}★${NC} ${BOLD}BATTLEFIELD 1942 · DEDICATED SERVER${NC} ${YELLOW}★${NC}  ${C_STEEL}[CentOS Stream 9]${NC}"
...
if [ ! -f "/etc/bf1942_deps_installed" ]; then
    ...
    # Enable EPEL and CRB for compat packages
