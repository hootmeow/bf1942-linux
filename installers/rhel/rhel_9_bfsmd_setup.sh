#!/usr/bin/env bash
# ---------------------------------------------------------------------------
#  Battlefield 1942 Linux Server - Unified Setup Script (RHEL)
#
#  Tested on:
#    - Red Hat Enterprise Linux 9
#
#  Usage:
#    Interactive: sudo ./rhel_9_bfsmd_setup.sh [instance_name]
#    Unattended:  sudo ./rhel_9_bfsmd_setup.sh <instance_name> --yes [options]
#    All options: sudo ./rhel_9_bfsmd_setup.sh --help
#
...
if ! command -v dnf &> /dev/null; then
    log_error "This script requires dnf (RHEL 9)."
    exit 1
fi
...
if [ -f /etc/os-release ]; then
    . /etc/os-release
    if [ "$ID" != "rhel" ]; then
        log_warn "This script is intended for RHEL. Detected: ${PRETTY_NAME:-unknown}"
...
echo ""
echo -e "${C_ARMY}   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "   ${YELLOW}★${NC} ${BOLD}BATTLEFIELD 1942 · DEDICATED SERVER${NC} ${YELLOW}★${NC}  ${C_STEEL}[RHEL 9]${NC}"
...
if [ ! -f "/etc/bf1942_deps_installed" ]; then
    log_info "First-time setup: Enabling repos and installing 32-bit libraries..."

    # Enable EPEL and CRB (CodeReady Linux Builder) for compat packages
    ...
    dnf config-manager --enable crb 2>/dev/null || \
        dnf config-manager --enable codeready-builder-for-rhel-9-$(arch)-rpms 2>/dev/null || \
        log_warn "Could not enable CRB — some packages may be unavailable"

    log_info "Installing modern 32-bit libraries..."
    dnf install -y \
        glibc.i686 libstdc++.i686 libgcc.i686 \
        zlib.i686 libcurl.i686 libX11.i686 \
